...
for oos_year in tqdm(range(1990, 2025), desc='Overall', unit='year'):
    ...
    # standardise on full training data, apply to validation
    scaler_annual = StandardScaler()
    Xtrain_sc = scaler_annual.fit_transform(Xtrain)
    Xval_sc = scaler_annual.transform(Xval)
    ...
    X0_tr, y0_tr = Xtrain_sc[train_states_tune == 0], ytrain[train_states_tune == 0]
    X1_tr, y1_tr = Xtrain_sc[train_states_tune == 1], ytrain[train_states_tune == 1]
    X0_val, y0_val = Xval_sc[val_states_tune == 0], yval[val_states_tune == 0]
    X1_val, y1_val = Xval_sc[val_states_tune == 1], yval[val_states_tune == 1]

    # tune number of components for g0 (bull regime)
    best_val_r2_0 = -np.inf
    best_n_0 = 1
    for n in range(1, min(X0_tr.shape[1], 30)):
        pipe = Pipeline([('pca', PCA(n_components=n)), ('lr', LinearRegression())])
        pipe.fit(X0_tr, y0_tr)
        yhat = pipe.predict(X0_val)
        r2 = 1 - np.sum((y0_val - yhat)**2) / np.sum(y0_val**2)
        if r2 > best_val_r2_0:
            best_val_r2_0 = r2
            best_n_0 = n

    # tune number of components for g1 (bear regime)
    best_val_r2_1 = -np.inf
    best_n_1 = 1
    for n in range(1, min(X1_tr.shape[1], 30)):
        pipe = Pipeline([('pca', PCA(n_components=n)), ('lr', LinearRegression())])
        pipe.fit(X1_tr, y1_tr)
        yhat = pipe.predict(X1_val)
        r2 = 1 - np.sum((y1_val - yhat)**2) / np.sum(y1_val**2)
        if r2 > best_val_r2_1:
            best_val_r2_1 = r2
            best_n_1 = n

    print(f'{oos_year} | g0: n={best_n_0} | g1: n={best_n_1}', flush=True)
    ...
    for month in tqdm(oos_months, desc=f'{oos_year}', unit='month', leave=False):
        ...
        # standardise within each month using training data statistics
        ...
        g0 = Pipeline([('pca', PCA(n_components=best_n_0)), ('lr', LinearRegression())])
        g0.fit(X0_sc, y0)

        g1 = Pipeline([('pca', PCA(n_components=best_n_1)), ('lr', LinearRegression())])
        g1.fit(X1_sc, y1)

        yhat_0 = g0.predict(X_pred_sc)
        yhat_1 = g1.predict(X_pred_sc)
        ...
        complexity_records.append({
            'month': month,
            'year': oos_year,
            'n_components_0': best_n_0,
            'n_components_1': best_n_1,
            'p_t': p_t,
        })
...
pd.DataFrame(complexity_records).to_parquet(f'{complexity_dir}/pcr_hmm_complexity.parquet', index=False)
...
print_results('PCR-HMM', r2, sharpe, mean_hl)
df_test[['permno', 'eom', 'ret_exc_lead1m', 'y_pred', 'y_pred_0', 'y_pred_1']].to_parquet(
    f'{pred_dir}/pcr_hmm.parquet', index=False)
save_metrics('pcr_hmm', r2, sharpe, mean_hl)
print("Finished.")