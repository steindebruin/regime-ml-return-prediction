...
# elastic net hyperparameters tuned separately per regime
...
for oos_year in tqdm(range(1990, 2025), desc='Overall', unit='year'):
    ...
    # tune alpha for g0 (bull regime)
    best_val_r2_0 = -np.inf
    best_alpha_0 = alpha_grid[0]
    for alpha in alpha_grid:
        m = ElasticNet(alpha=alpha, l1_ratio=l1_ratio, max_iter=5000, fit_intercept=True)
        m.fit(X0_tr, y0_tr)
    ...
    # tune alpha for g1 (bear regime)
    best_val_r2_1 = -np.inf
    best_alpha_1 = alpha_grid[0]
    for alpha in alpha_grid:
        m = ElasticNet(alpha=alpha, l1_ratio=l1_ratio, max_iter=5000, fit_intercept=True)
        m.fit(X1_tr, y1_tr)
    ...
    print(f'{oos_year} | g0: alpha={best_alpha_0:.2e} | g1: alpha={best_alpha_1:.2e}', flush=True)
    ...
    for month in tqdm(oos_months, desc=f'{oos_year}', unit='month', leave=False):
        ...
        yhat_0 = g0.predict(X_pred)
        yhat_1 = g1.predict(X_pred)
        ...
        complexity_records.append({
            'month': month,
            'year': oos_year,
            'alpha_0': best_alpha_0,
            'alpha_1': best_alpha_1,
            'n_nonzero_0': int(np.sum(g0.coef_ != 0)),
            'n_nonzero_1': int(np.sum(g1.coef_ != 0)),
            'p_t': p_t,
        })
...
pd.DataFrame(complexity_records).to_parquet(f'{complexity_dir}/enet_hmm_complexity.parquet', index=False)
...
print_results('ENet-HMM', r2, sharpe, mean_hl)
df_test[['permno', 'eom', 'ret_exc_lead1m', 'y_pred', 'y_pred_0', 'y_pred_1']].to_parquet(
    f'{pred_dir}/enet_hmm.parquet', index=False)
save_metrics('enet_hmm', r2, sharpe, mean_hl)
print("Finished.")