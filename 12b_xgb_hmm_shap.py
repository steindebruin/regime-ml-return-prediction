...
complexity = pd.read_parquet('results/complexity/xgb_hmm_complexity.parquet')
params_by_year = complexity.groupby('year')[
    ['lr_0', 'depth_0', 'n_est_0', 'lr_1', 'depth_1', 'n_est_1']].first()
...
for oos_year in tqdm(range(1990, 2025), desc='Overall', unit='year'):
    ...
    lr_0, depth_0, n_est_0 = p['lr_0'], int(p['depth_0']), int(p['n_est_0'])
    lr_1, depth_1, n_est_1 = p['lr_1'], int(p['depth_1']), int(p['n_est_1'])
    ...
    for month in tqdm(oos_months, desc=f'{oos_year}', unit='month', leave=False):
        ...
        if len(y0) > 10:
            g0 = xgb.XGBRegressor(
                n_estimators=n_est_0, learning_rate=lr_0, max_depth=depth_0,
        ...
        if len(y1) > 10:
            g1 = xgb.XGBRegressor(
                n_estimators=n_est_1, learning_rate=lr_1, max_depth=depth_1,
                objective='reg:squarederror', tree_method='hist',
                random_state=42, n_jobs=-1, verbosity=0)
            g1.fit(X1, y1)
...
for suffix, records in [('0', shap_records_0), ('1', shap_records_1), ('total', shap_records_tot)]:
    ...
    agg.to_parquet(f'{importance_dir}/xgb_hmm_importance_{suffix}.parquet', index=False)
