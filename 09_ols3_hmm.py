...
from utils import (data_path, R2oos, portfolio_metrics, save_metrics, print_results)
...
X_train_full = df.loc[train_mask, ols3_cols].values.astype(np.float64)
...
for oos_year in tqdm(range(1990, 2025), desc='Overall', unit='year'):
    ...
    # map training observations to their HMM regime for this OOS year
    ...
    g0 = LinearRegression().fit(X0, y0) if len(y0) > len(ols3_cols) else None
    g1 = LinearRegression().fit(X1, y1) if len(y1) > len(ols3_cols) else None
    ...
    for month in tqdm(oos_months, desc=f'{oos_year}', unit='month', leave=False):
        ...
        X_pred = df.loc[df['eom'] == month, ols3_cols].values.astype(np.float64)
...
print_results('OLS3-HMM', r2, sharpe, mean_hl)
df_test[['permno', 'eom', 'ret_exc_lead1m', 'y_pred', 'y_pred_0', 'y_pred_1']].to_parquet(
    f'{pred_dir}/ols3_hmm.parquet', index=False)
save_metrics('ols3_hmm', r2, sharpe, mean_hl)
print("Finished.")