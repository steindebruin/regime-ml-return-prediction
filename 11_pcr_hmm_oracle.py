...
for oos_year in tqdm(range(1990, 2025), desc='Overall', unit='year'):
    ...
    for month in tqdm(oos_months, desc=f'{oos_year}', unit='month', leave=False):
        ...
        y_hat = p_oracle * g0.predict(X_pred_sc) + (1 - p_oracle) * g1.predict(X_pred_sc)
...
pd.DataFrame(complexity_records).to_parquet(
    f'{complexity_dir}/pcr_hmm_oracle_complexity.parquet', index=False)
...
print_results('PCR-HMM-ORACLE', r2, sharpe, mean_hl)
df_test[['permno', 'eom', 'ret_exc_lead1m', 'y_pred']].to_parquet(
    f'{pred_dir}/pcr_hmm_oracle.parquet', index=False)
save_metrics('pcr_hmm_oracle', r2, sharpe, mean_hl)
print("Finished.")